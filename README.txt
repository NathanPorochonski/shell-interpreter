...
To run the shell, type "./shell" in a terminal of the directory with the executable. Use the "path" command to display the current path environment. To add a path use "path + [path name]" and to remove one use "path - [path name]". Use "cd [directory name]" to change directories. Type the name of the executable to run it. The path of the executable must be included in the path environment to run it.
